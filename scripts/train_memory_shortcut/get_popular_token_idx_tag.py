# 根据训练时大部分被打什么标签，得到测试集or训练集的大众tag文件，和mt对应，得到01文件，0代表小众或持平，1代表大众, -1 代表没在训练集出现过
import json
...
qe_test_popular_tag_path = test_prefix + "test20.mt_popular_tag"
...
with open(qe_test_mt_path, 'r', encoding='utf-8') as f_mt, open(qe_test_gold_tag_path, 'r', encoding='utf-8') as f_tag, \
    open(qe_test_popular_tag_path, 'w', encoding='utf-8') as f_save:
    for line_mt, line_tag in zip(f_mt.readlines(), f_tag.readlines()):
        ...
        for token, tag in zip(line_mt, line_tag):
            if token in train_tag_stat:
                if tag == 'OK' and train_tag_stat[token]['ok_ratio'] <= 0.5: line_popular.append(0)
                elif tag == 'BAD' and train_tag_stat[token]['ok_ratio'] >= 0.5: line_popular.append(0)
                else: line_popular.append(1)
            else:
                line_popular.append(-1)
        f_save.write(' '.join(map(str, line_popular)) + '\n')

# python3 scripts/get_popular_token_idx_tag.py