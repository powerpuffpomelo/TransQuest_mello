...
from mlqe_word_level.RobertaForTranslationAndQE import XLMRobertaForTranslationAndQE
from mlqe_word_level.RobertaForProto import XLMRobertaForProto
...
class MicroTQForTLM(MicroTransQuestModel):
    def __init__(
            self,
            model_type,
            model_name,
    ...
    def train_model(
        ...
        global_step, training_details = self.train(
            train_data, output_dir, show_running_loss=show_running_loss, eval_data=eval_data, **kwargs
        )
    ...
    def train(self, train_data, output_dir, show_running_loss=True, eval_data=None, verbose=True, **kwargs):
        """
        Trains the model on train_dataset.
        ...
        for _ in train_iterator:   # 每个epoch
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                # true_predict_count += correction_list.count(True)
                # total_predict_count += len(correction_list)
...
class MicroTQForTLMQE(MicroTransQuestModel):
    def __init__(
            self,
            model_type,
            model_name,
        ...
        MODEL_CLASSES = {
            ...
            "distilbert": (DistilBertConfig, DistilBertForTokenClassification, DistilBertTokenizer),
            "roberta": (RobertaConfig, RobertaForTokenClassification, RobertaTokenizer),
            "xlmroberta": (XLMRobertaConfig, XLMRobertaForTranslationAndQE, XLMRobertaTokenizer),
        }
        ...
        if self.args.qe_lambda:
            self.args.config['qe_lambda'] = self.args.qe_lambda
    ...
    def train_model(
        ...
        train_data_translation = pd.DataFrame(train_data, columns=['source', 'pe'])
        train_data_qe = pd.DataFrame(train_data, columns=['source', 'target', 'source_tags', 'target_tags'])

        train_data_translation = prepare_data(train_data_translation, self.args)
        train_data_qe = prepare_data(train_data_qe, self.args)

        eval_data = prepare_data(eval_data, self.args)

        self._move_model_to_device()

        train_dataset_qe = self.load_and_cache_examples(train_data_qe)

        os.makedirs(output_dir, exist_ok=True)

        global_step, training_details = self.train(
            train_data_translation, train_dataset_qe, output_dir, show_running_loss=show_running_loss, eval_data=eval_data, **kwargs
        )
    ...
    def train(self, train_data_translation, train_dataset_qe, output_dir, show_running_loss=True, eval_data=None, verbose=True, **kwargs):
        """
        Trains the model on train_dataset.
        ...
        train_sampler_qe = RandomSampler(train_dataset_qe)
        train_dataloader_qe = DataLoader(
            train_dataset_qe,
            sampler=train_sampler_qe,
            batch_size=args.train_batch_size,
            num_workers=self.args.dataloader_num_workers,
        )

        t_total = len(train_dataloader_qe) // args.gradient_accumulation_steps * args.num_train_epochs
        ...
        if args.model_name and os.path.exists(args.model_name):
            try:
                ...
                if len(checkpoint_suffix) > 2:
                    ...
                    checkpoint_suffix = checkpoint_suffix[-1]
                global_step = int(checkpoint_suffix)
                epochs_trained = global_step // (len(train_dataloader_qe) // args.gradient_accumulation_steps)
                steps_trained_in_current_epoch = global_step % (
                        len(train_dataloader_qe) // args.gradient_accumulation_steps
                )
        ...
        for _ in train_iterator:
            ...
            # 动态mask，就在这里加载dataloader好了
            train_dataset_translation = self.load_and_cache_examples(train_data_translation)
            train_sampler_translation = RandomSampler(train_dataset_translation)
            train_dataloader_translation = DataLoader(
                dataset=train_dataset_translation,
                sampler=train_sampler_translation,
                batch_size=args.train_batch_size,
                #collate_fn=self.load_and_cache_examples,
                num_workers=self.args.dataloader_num_workers,
            )
            ...
            batch_iterator_translation = tqdm(
                train_dataloader_translation,
                desc=f"Running Epoch {epoch_number} of {args.num_train_epochs}",
                disable=args.silent,
                mininterval=0,
            )
            batch_iterator_qe = tqdm(
                train_dataloader_qe,
                desc=f"Running Epoch {epoch_number} of {args.num_train_epochs}",
                disable=args.silent,
                mininterval=0,
            )
            for step, (batch_translation, batch_qe) in enumerate(zip(batch_iterator_translation, batch_iterator_qe)):

                if steps_trained_in_current_epoch > 0:
                    steps_trained_in_current_epoch -= 1
                    continue
                batch_translation = tuple(t.to(device) for t in batch_translation)  # input_ids, input_mask, segment_ids, label_ids
                batch_qe = tuple(t.to(device) for t in batch_qe)  # input_ids, input_mask, segment_ids, label_ids

                inputs_translation = self._get_inputs_dict(batch_translation)
                inputs_translation['translation_labels'] = inputs_translation.pop('labels')
                inputs_qe = self._get_inputs_dict(batch_qe)
                inputs_qe['token_qe_labels'] = inputs_qe.pop('labels')

                if self.args.fp16:
                    with amp.autocast():
                        # model outputs are always tuple in pytorch-transformers (see doc)
                        outputs_translation = model(**inputs_translation)
                        loss_translation = outputs_translation[0]
                        outputs_qe = model(**inputs_qe)
                        loss_qe = outputs_qe[0]
                else:
                    # forward在这里
                    outputs_translation = model(**inputs_translation)
                    loss_translation = outputs_translation[0]
                ...
                if args.n_gpu > 1:
                    loss_translation = loss_translation.mean()  # mean() to average on multi-gpu parallel training
                    loss_qe = loss_qe.mean()

                loss = loss_translation * 0.1 * (1 - self.args.qe_lambda) + loss_qe * self.args.qe_lambda

                current_loss_translation = loss_translation.item()
                current_loss_qe = loss_qe.item()
                current_loss = current_loss_translation * 0.1 * (1 - self.args.qe_lambda) + current_loss_qe * self.args.qe_lambda

                if show_running_loss:
                    batch_iterator_translation.set_description(
                        f"Epochs {epoch_number}/{args.num_train_epochs}. Running Translation Loss: {current_loss_translation:9.4f}"
                    )
                    batch_iterator_qe.set_description(
                        f"Epochs {epoch_number}/{args.num_train_epochs}. Running QE Loss: {current_loss_qe:9.4f}"
                    )
    ...
    def evaluate(self, eval_dataset, output_dir, verbose=True, silent=False, wandb_log=True, **kwargs):
        ...
        for batch in tqdm(eval_dataloader, disable=args.silent or silent, desc="Running Evaluation"):
            ...
            with torch.no_grad():
                inputs = self._get_inputs_dict(batch)
                inputs['token_qe_labels'] = inputs.pop('labels')
            ...
            if preds is None:
                preds = logits.detach().cpu().numpy()
                out_label_ids = inputs["token_qe_labels"].detach().cpu().numpy()
                out_input_ids = inputs["input_ids"].detach().cpu().numpy()
                out_attention_mask = inputs["attention_mask"].detach().cpu().numpy()
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                out_label_ids = np.append(out_label_ids, inputs["token_qe_labels"].detach().cpu().numpy(), axis=0)
                out_input_ids = np.append(out_input_ids, inputs["input_ids"].detach().cpu().numpy(), axis=0)
                out_attention_mask = np.append(
                    out_attention_mask, inputs["attention_mask"].detach().cpu().numpy(), axis=0,
                )
...
class MicroTQForProto(MicroTransQuestModel):
    def __init__(
            self,
            model_type,
            model_name,
        ...
        MODEL_CLASSES = {
            ...
            "distilbert": (DistilBertConfig, DistilBertForTokenClassification, DistilBertTokenizer),
            "roberta": (RobertaConfig, RobertaForTokenClassification, RobertaTokenizer),
            "xlmroberta": (XLMRobertaConfig, XLMRobertaForProto, XLMRobertaTokenizer),
        }
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if self.args.fp16:
                    ...
                    # forward在这里
                    outputs = model(**inputs)
                    proto_output = outputs[0]  # [batch_size, max_seq_len, hidden_dim]
                    labels = inputs['labels'] # [batch_size, max_seq_len] 0 ok 1 bad

                    support_size = int(proto_output.size(0) * self.args.support_ratio)

                    support_output = proto_output[:support_size]
                    query_output = proto_output[support_size:]
                    ...
                    support_output = support_output.view(-1, support_output.size(-1))
                    query_output = query_output.view(-1, query_output.size(-1))
                    
                    centers = []
                    neg_dists = []

                    for l in range(self.num_labels):
                        mask = (support_labels == l)
                        center = support_output[mask].mean(dim=0)
                        centers.append(center)
                        neg_dist = -((query_output - center) ** 2).sum(dim=-1)
                        neg_dists.append(neg_dist)
                    centers = torch.stack(centers, dim=0)   # [2, hidden_size] ok bad的center向量
                    neg_dists = torch.stack(neg_dists, dim=-1)   # [query_size * max_seq_len, 2]  可以看做logit

                    if self.cnt == 1:
                        self.centers = centers
                    else:
                        self.centers = (self.centers * (self.cnt - 1) + centers) / self.cnt

                    loss_func = CrossEntropyLoss()
                    loss = loss_func(neg_dists, query_labels)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        self.save_model(output_dir_current, optimizer, scheduler, model=model)
                        torch.save(self.centers, output_dir + 'centers_' + str(global_step))
        ...
        for batch in tqdm(eval_dataloader, disable=args.silent or silent, desc="Running Evaluation"):
            ...
            with torch.no_grad():
                ...
                if self.args.fp16:
                    ...
                    outputs = model(**inputs)
                    proto_output = outputs[0]  # [batch_size, max_seq_len, hidden_dim]
                    batch_size = proto_output.size(0)
                    proto_output = proto_output.view(-1, proto_output.size(-1))
                    labels = inputs['labels'].view(-1) # [batch_size, max_seq_len] 0 ok 1 bad

                    neg_dists = []

                    for l in range(self.num_labels):
                        neg_dist = -((proto_output - self.centers[l]) ** 2).sum(dim=-1)
                        neg_dists.append(neg_dist)
                    neg_dists = torch.stack(neg_dists, dim=-1)   # [query_size * max_seq_len, 2]  可以看做logit
                    
                    loss_func = CrossEntropyLoss()
                    tmp_eval_loss = loss_func(neg_dists, labels)
                    logits = neg_dists.view(batch_size, -1, self.num_labels)
    ...
    def predict(self, to_predict, split_on_space=True):
        ...
        model = self.model
        args = self.args
        centers = torch.load(args.centers_path).to(device)
            ...
            for batch in tqdm(eval_dataloader, disable=args.silent, desc="Running Prediction"):
                ...
                with torch.no_grad():
                    ...
                    if self.args.fp16:
                        ...
                        for l in range(self.num_labels):
                            neg_dist = -((proto_output - centers[l]) ** 2).sum(dim=-1)
                            neg_dists.append(neg_dist)
                        neg_dists = torch.stack(neg_dists, dim=-1)   # [query_size * max_seq_len, 2]  可以看做logit
