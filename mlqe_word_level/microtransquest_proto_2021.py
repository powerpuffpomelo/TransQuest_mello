...
from mlqe_word_level.microtransquest_config.microtransquest_config_2021 import path_prefix, TRAIN_SOURCE_FILE, \
    TRAIN_SOURCE_TAGS_FILE, \
    TRAIN_TARGET_FILE, \
    TRAIN_TARGET_TAGS_FLE, MODEL_TYPE, MODEL_NAME, microtransquest_config, TEMP_DIRECTORY, SEED, \
    DEV_SOURCE_FILE, DEV_TARGET_FILE, DEV_SOURCE_TAGS_FILE, DEV_TARGET_TAGS_FLE, \
    TEST_PATH, TEST_SOURCE_FILE, TEST_TARGET_FILE
from mlqe_word_level.run_model import MicroTQForProto
...
temp_prefix = "checkpoints/train_result_21_" + lang_pair
if args.save_name is not None:
    temp_prefix = "checkpoints/train_result_21_proto_" + args.save_name + "_" + lang_pair
...
microtransquest_config['support_ratio'] = 0.75
...
raw_dev_df = reader(DEV_PATH, DEV_SOURCE_FILE, DEV_TARGET_FILE, DEV_SOURCE_TAGS_FILE,
                    DEV_TARGET_TAGS_FLE)
raw_test_df = reader(path=TEST_PATH, source_file=TEST_SOURCE_FILE, target_file=TEST_TARGET_FILE)
...
for i in range(microtransquest_config["n_fold"]):
    ...
    if microtransquest_config["evaluate_during_training"]:
        ...
        model = MicroTQForProto(MODEL_TYPE, MODEL_NAME, labels=["OK", "BAD"], args=microtransquest_config)
        model.train_model(raw_train, eval_data=raw_eval)
        model = MicroTQForProto(MODEL_TYPE, microtransquest_config["best_model_dir"], labels=["OK", "BAD"],
