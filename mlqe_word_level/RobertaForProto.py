...
class RobertaForProtoOutput(ModelOutput):
    """
    ...
    proto_output: Optional[torch.FloatTensor] = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None


class RobertaForProto(RobertaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.proto_layer = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(
    ...
    ) -> Union[Tuple[torch.Tensor], RobertaForProtoOutput]:
        r"""
        ...
        proto_output = self.proto_layer(sequence_output)

        if not return_dict:
            output = (proto_output) + outputs[2:]
            return output
        
        return RobertaForProtoOutput(
...
class XLMRobertaForProto(RobertaForProto):
    """
